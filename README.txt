...
Purpose: 
Reliable File Transfer Protocol with server and client application using socket programming. 
Client can upload(put) and download(get) file from the server. 
...
Setup Instructions:
	...
	6.	Setup for client.py
		a.	Place client.py and supportFunc.py in the same parent directory 
			...
			Any file in the parent directory(.) , same directory where 
			client.py is (./client.py) can be put to server.
		b.	When you get a file from the server, client will create subdirectory 
			under the same parent directory with 
			client.py call DLFromServer/ (./DLFromServer/) 
			if the subdirectory doesn’t already exists, and client 
			will places the get files inside that subdirectory.
	7.	Setup for server.py
		a.	Place client.py and supportFunc.py in the same parent directory. 
			Within that same parent directory create a 
			subdirectory name fileOnServer (./fileOnServer/) 
			and place any file that you want to send to the client inside the 
